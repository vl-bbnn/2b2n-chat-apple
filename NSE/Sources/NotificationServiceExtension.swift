...
actor NotificationServiceExtensionActor {
    ...
    init() {
        ...
        // the target configuration will fail, so wait for the delegate method to be called
        // and bail out there instead.
    ...
    func handle(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) async {
        ...
        // So lets skip processing the notification and deliver a special "offline" notification as a workaround.
        guard Self.targetConfiguration.withLock({ $0 != nil }) else {
            ...
            if await Self.hasHandledFirstNotificationSinceBoot {
                ...
                return contentHandler(receivedWhileOfflineContent(for: request))
            }
        }
    ...
    /// Builds a generic notification informing the user that they have one or more new messages.
    private func receivedWhileOfflineContent(for originalRequest: UNNotificationRequest) -> UNNotificationContent {
